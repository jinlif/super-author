...
function convertMessages(messages: AgentMessage[]): OpenAIMessage[] {
  ...
  for (const msg of messages) {
    if (msg.role === 'user') {
      ...
      const thinkingBlocks = msg.content.filter((b) => b.type === 'thinking') as {
        type: 'thinking'
        text: string
      }[]
      const toolUseBlocks = msg.content.filter((b) => b.type === 'tool_use') as {
        type: 'tool_use'
        id?: string
        name?: string
      ...
      const text = textBlocks.map((b) => b.text).join('\n')
      const reasoningContent = thinkingBlocks.map((b) => b.text).join('\n') || undefined
...
export class OpenAIProvider implements IProvider {
  ...
  async *createMessage(
    ...
    const baseParams = {
      model: this.model,
      messages: [{ role: 'system' as const, content: systemPrompt }, ...apiMessages],
